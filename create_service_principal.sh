#!/bin/bash
# From https://github.com/kubernetes/kubernetes-anywhere/blob/master/phase1/azure/create-azure-service-principal.sh
...
case "${output_format:-}" in
	"json")
		output=".tenant_id=\"${tenant_id}\"|.subscription_id=\"${subscription_id}\"|.client_id=\"${app_url}\"|.client_secret=\"${client_secret}\"|.app_id=\"${app_id}\""
		jq -n "${output}"
		;;
	"text")
